    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col gap-5 mt-8">
        ...
        {turnos.map(turno => {
          ...
          return (
            <div key={turno.id} className={`bg-white border rounded-xl p-6 shadow-sm transition-all hover:shadow-md relative overflow-hidden ${isClosed ? 'border-zinc-200' : 'border-amber-200 shadow-amber-500/5'
              }`}>

              {/* Barra lateral indicadora */}
              <div className={`absolute left-0 top-0 bottom-0 w-2 ${!isClosed ? 'bg-amber-400' :
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 pl-4">
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`px-2.5 py-1 text-[10px] font-black tracking-widest uppercase rounded-full border ${isClosed ? 'bg-zinc-100 text-zinc-600 border-zinc-200' : 'bg-amber-50 text-amber-700 border-amber-200 animate-pulse'
                      }`}>
                      {turno.estado}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-zinc-100 pl-4">
                <div className="flex flex-col p-4 rounded-xl bg-zinc-50/80 border border-zinc-100/50">
                  ...
                  <p className="text-xs text-zinc-400 mt-2">Efectivo físico contabilizado al momento de cerrar caja.</p>
                </div>
                ...
                <div className={`flex flex-col p-4 rounded-xl border shadow-sm ${!isClosed ? 'border-zinc-100 bg-white' :
                    isBalanced ? 'bg-emerald-50 border-emerald-200' :
                      'bg-rose-50 border-rose-200'
                  }`}>
                  <span className={`text-xs font-bold uppercase tracking-wider mb-1 ${!isClosed ? 'text-zinc-400' :
                      isBalanced ? 'text-emerald-700' : 'text-rose-700'
                    }`}>Diferencia</span>
                  <span className={`text-3xl font-black tracking-tight ${!isClosed ? 'text-zinc-300' :
                      isBalanced ? 'text-emerald-600' : 'text-rose-600'
                    }`}>
                    {!isClosed ? '-.--' : `${diferencia > 0 ? '+' : ''}${isBalanced ? '0.00' : diferencia.toFixed(2)}`}
                  </span>
                  <p className={`text-xs mt-2 font-medium ${!isClosed ? 'text-zinc-400' :
                      isBalanced ? 'text-emerald-600' : 'text-rose-600'
                    }`}>
                            ...
                            return (
                              <tr key={item.id} className="hover:bg-zinc-50/50">
                                ...
                                <td className="px-4 py-3 text-center">
                                  <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-bold ${!hasDiscrepancy ? 'bg-zinc-100 text-zinc-500' :
