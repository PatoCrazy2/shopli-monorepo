...
import { Loader2, AlertTriangle, CheckCircle2, PackageX, PackageCheck, X } from "lucide-react";
...
export function ToggleProductButton({
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await toggleProduct(productId, isActive);
      if (res?.error) {
        setError(res.error);
        setIsPending(false);
      } else {
  ...
  return (
    <>
      <button
        type="button"
        onClick={handleOpen}
        className={`inline-flex items-center justify-center h-8 px-3 rounded-lg text-xs font-bold transition-all duration-200 shadow-sm ${
          isActive
            ? "bg-zinc-100 text-zinc-700 hover:bg-red-50 hover:text-red-600 hover:border-red-200 border border-transparent dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-red-950/40 dark:hover:text-red-400"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          >
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">Gestión de Catálogo</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="text-sm text-zinc-600 dark:text-zinc-400 space-y-2">
              <p>
                Estás a punto de {isActive ? "desactivar" : "reactivar"}{" "}
                <span className="font-semibold text-zinc-900 dark:text-white">{productName}</span>.
              </p>
              {isActive ? (
                <div className="p-3 bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 text-xs space-y-1 text-zinc-600 dark:text-zinc-400">
                  ...
                  <ul className="list-disc list-inside space-y-0.5 text-zinc-500 pl-1">
                    <li>Se ocultará de la lista activa del catálogo para evitar ruido visual.</li>
                    <li>No estará disponible para cobro en la app de Punto de Venta (POS).</li>
                    <li>Sus variantes asociadas también quedarán inactivas.</li>
                ...
                <div className="p-3 bg-emerald-50/60 dark:bg-emerald-950/20 rounded-xl border border-emerald-100 dark:border-emerald-900 text-xs space-y-1 text-emerald-800 dark:text-emerald-300">
                  ...
                  <p className="text-zinc-600 dark:text-zinc-400">
                    El producto y sus variantes volverán al catálogo activo y se sincronizarán con los puntos de venta.
                  </p>
                </div>
              )}
            </div>
